...
void EnemyGolem::UpdateOnlyTransform(float elapsedTime)
{
    // オブジェクト行列を更新
    UpdateTransform();
...
RenderContext EnemyGolem::SetRenderContext(const RenderContext& rc)
{
...
void EnemyGolem::SetRamdomTargetPosition()
{
...
void EnemyGolem::MoveToTarget(float elapsedTime, float speedRate)
{
    // ターゲット方向へ進行ベクトルを算出
...
Player* EnemyGolem::searchPlayer()
{
    // プレイヤーとの高低差を考慮して3Dでの距離判定をする
    const DirectX::XMFLOAT3& playerPosition = Player::Instance().GetPosition();
...
void EnemyGolem::TransitionWanderState()
{
    ...
    // 歩きアニメーション再生
    model->PlayAnimation(Animation::Anim_Walking, true, 0.7f);
}

// 徘徊ステート更新処理
void EnemyGolem::UpdateWanderState(float elapsedTime)
{
    // 目標地点までXZ平面での距離判定
    float vx = targetPosition.x - param.position.x;
    float vz = targetPosition.z - param.position.z;
...
// 待機ステートへ遷移
void EnemyGolem::TransitionIdleState()
{
    state = State::Idle;
...
// 待機ステート更新処理
void EnemyGolem::UpdateIdleState(float elapsedTime)
{
    // タイマー処理
...
// 追跡ステート遷移
void EnemyGolem::TransitionPursuitState()
{
    state = State::Pursuit;
    moveSpeed = 3.0f;
    stateTimer = Mathf::RandomRange(3.0f, 5.0f);
...
//追跡ステート更新処理
void EnemyGolem::UpdatePursuitState(float elapsedTime)
{
    // 目標地点をプレイヤー位置に設定
